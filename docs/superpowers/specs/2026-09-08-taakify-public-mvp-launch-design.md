...
- Catch production errors (server and client) via self-hosted, open-source
  ...
  open source, no proprietary services" principle (R2 remains the one
  documented exception; a second proprietary exception is avoided here).
...
| ----------------- | ----------------------------------------- | -------------------------------------------------------- |
| Edge/TLS          | Caddy                                     | Reverse proxy + automatic Let's Encrypt HTTPS for the domain |
...
```
Internet
   │  :443 (HTTPS), :80 (redirect to 443)
   ▼
 Caddy  ──────────────┬──────────────┐
   │ /api/*, /electric/* proxy       │ everything else
   ▼                                  ▼
...
Caddy owns TLS termination and domain routing; it is the only container
exposed to the internet (22/SSH aside). All other containers communicate
over the compose-internal Docker network, matching the existing dev
Postgres/Electric relationship but now with no ports published to the host
...
path (HTTPS through Caddy instead of the Vite dev proxy) and that
`BETTER_AUTH_URL` / cookie flags now reflect a real HTTPS origin
(`Secure`, `SameSite=Lax` — verify this differs from whatever the dev
config currently assumes for `http://localhost`).
...
- **`docker-compose.prod.yml`** — Postgres (named volume for data),
  Electric, api, web, Caddy, GlitchTip (plus GlitchTip's own dependencies —
  it needs its own Postgres + Redis; check current GlitchTip docker-compose
  reference at implementation time and either run a second lightweight
  Postgres or point it at the same instance in a separate database), and
  the backup sidecar.
- **`.env.prod.example`** — production secrets template: `DATABASE_URL`,
  ...
  client-exposed GlitchTip DSN for the web build.
...
- **Deploy runbook** — a new `docs/deploy.md`: provision VM → configure
  firewall (see §5) → clone repo → populate `.env.prod` → `docker compose
  -f docker-compose.prod.yml up -d` → run `pnpm migrate` against the prod
  DB → point DNS A/AAAA record at the VM → verify HTTPS via Caddy →
  smoke-test with Playwright (§7) before announcing the URL to anyone.

## 5. Security Hardening Pass

- **VM firewall** — only 80, 443, and SSH open (SSH key-only auth,
  consider a non-default port and/or fail2ban given the box is now
  internet-facing).
